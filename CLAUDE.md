...
**Restart race condition fix:** `reset_crash_detection()` is called BEFORE `container.start()` (not after), and a preliminary `record_restart(datetime.now(), reason)` is written immediately after start (before sleep) to close the unplanned-detection window. After sleep(2/3), the record is updated with the precise Docker `started_at`. This prevents false Crash and Unplanned badges during the startup window. Applies to both `start_telegraf()` and `generate_config()` in `telegraf.py`.
...
- 123 tests, all green -- run with `source venv/bin/activate && python -m pytest tests/ -v`
- Test files: `test_telegraf_config.py` (27), `test_toml_filters.py` (30), `test_mqtt_client.py` (12), `test_config_store.py` (31), `test_system_monitor.py` (29) -- see MEMORY.md for details
